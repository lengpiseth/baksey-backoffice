...
namespace Tools\GeneratorBundle\Generator;
...
class DoctrineCrudGenerator extends Generator
{
     ...
     */
    public function generate(BundleInterface $bundle, $document, ClassMetadataInfo $metadata, $format, $routePrefix, $needWriteActions, $isAdmin)
    {
        $this->routePrefix     = $routePrefix;
        $this->routeNamePrefix = preg_replace('/[^\w]+/', '_', $routePrefix);
        $this->actions         = $needWriteActions ? array('index', 'show', 'new', 'edit', 'duplicate', 'delete') : array('index', 'show');
        ...
        if($isAdmin == true) {
            $dir = sprintf('%s/Resources/views/%s', $this->bundle->getPath().'/../../../app', str_replace('\\', DIRECTORY_SEPARATOR, $this->bundle->getNamespace().'\\Admin\\'.$this->document));
        }
        else {
            $dir = sprintf('%s/Resources/views/%s', $this->bundle->getPath().'/../../../app', str_replace('\\', DIRECTORY_SEPARATOR, $this->bundle->getNamespace().$this->document));
        }
